...
export const runInit = async () => {
  try {
    ...
    if (!isEmptyPath(config.currentPath)) {
      logger.error(
        'The path is not empty. Please choose an empty directory to initialize the documentation project.',
      );
      exit();
    }
    ...
    for (const mod of selectedModules) {
      ...
      const indexContent = getBlockFromFile(
        `${config.defaultsPath}/templates/modules_index.md`,
        mod,
      );
      createFile(`${config.currentPath}/${mod}/index.md`, indexContent);
      // Move templates into the templates folder
      copyFile(
        `${config.defaultsPath}/templates/_TEMPLATE_${mod}.md`,
        `${config.currentPath}/templates/_TEMPLATE_${mod}.md`,
      );
