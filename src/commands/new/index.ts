...
const newDoc = async () => {
  try {
    ...
    if (!existIndex && !existTemplatesFolder) {
      logger.error(
        "No documentation project found in the current directory. Please run 'devdocs init' first.",
      );
      exit();
    }
    ...
    // User parameters
    const parameters = getUserParameters(
      `${config.currentPath}/templates/_TEMPLATE_${selectedModule}.md`,
    );

    // Add fixed parameters if not present
    const fixedParameters = config.fixedParams;
    for (const fixedParam of fixedParameters) {
    ...
    // Auto parameters
    const autoParameters = getAutoParameters(
      `${config.currentPath}/templates/_TEMPLATE_${selectedModule}.md`,
    );
    ...
    copyFile(
      `${config.currentPath}/templates/_TEMPLATE_${selectedModule}.md`,
      `${config.currentPath}/${selectedModule}/${fileName}.md`,
    );
    // Replace parameters
    ...
    addDocLinkToIndex(
      `${config.currentPath}/${selectedModule}/index.md`,
      `${fileName}.md`,
      answers['title'] || 'untitled',
    );

    logger.info(
      `Document created successfully => ${config.currentPath}/${selectedModule}/${fileName}.md.`,
    );
...
const newModule = async () => {
  try {
    ...
    logger.info(
      `Template created => ${config.currentPath}/templates/_TEMPLATE_${selectedModule}.md.`,
    );
